...
def extract_all_csv_columns(mappings_dict):
    ...
    for value in mappings_dict.values():
        if isinstance(value, dict):
            csv_columns.extend([v for v in value.values() if v]) 
        else:
            if value:
...
def audit_metrics(schema: dict, headers: list[str], mapping: dict, rows: list[dict]):
    ...
    # Mapped tables: Ignore "extras" and check if at least one non-null mapping
    mapped_tables = [
        table for table, cols in mapping.items()
        if table != "extras" and any(val for val in cols.values())
    ]

    # Mapped columns (CSV columns) where schema_col is valid and value is not null
    mapped_columns = {
    ...
    # Mapped schema columns to compute what's missing
    ...
    # Extra columns = in CSV header but not in expected OR not used in mapping
    ...
    # Add extras from mapping['extras'] if they are not mapped to schema fields
    ...
    # Total columns = all non-null mapped CSV columns from all mappings
    total_columns = {
        col
        for table, cols in mapping.items()
